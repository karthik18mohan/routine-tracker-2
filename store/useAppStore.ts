...
type Store = AppState & {
  ...
  getOrCreateRemoteMonthId: (year: number, month: number) => Promise<string | null>;
...
export const useAppStore = create<Store>()((set, get) => ({
  ...
  refreshProfiles: async (preferredUserId) => {
    ...
    if (users.length === 0) {
      ...
      const { error: insertProfileError } = await createProfileInSupabase(
        supabaseUserId,
        newProfile
      );
      if (insertProfileError) {
        ...
        set({
          supabaseError: `Create default profile: ${
            insertProfileError.message ?? "Unknown error"
          }`,
          isProfilesLoading: false
        });
        return;
    ...
    const selectedUserId =
      desiredSelection && users.some((userProfile) => userProfile.id === desiredSelection)
        ? desiredSelection
        : users[0]?.id ?? null;
    set({
      users,
  ...
  refreshMonth: async (year, month) => {
    ...
    if (!selectedUserId || !get().supabaseReady) {
      return null;
    }
  ...
  getOrCreateRemoteMonthId: async (year, month) => {
    await get().ensureMonth(year, month);
    const key = buildMonthKey(year, month);
    const selectedUserId = get().selectedUserId;
    const monthState = selectedUserId ? get().monthsByUser[selectedUserId]?.[key] : null;
    return monthState?.id ?? null;
  },
  ...
  addUser: async (name) => {
    ...
    if (get().supabaseReady && get().supabaseUserId) {
      const { error } = await createProfileInSupabase(get().supabaseUserId, user);
      if (error) {
  ...
  toggleDailyCheck: async (habitId, dayIndex) => {
    ...
    const remoteMonthId = await get().getOrCreateRemoteMonthId(selectedYear, selectedMonth);
    if (!remoteMonthId) {
      return;
    }
  ...
  updateNotes: async (notes) => {
    ...
    if (get().supabaseReady) {
      const { error } = await updateMonthNotes(remoteMonthId, notes);
      if (error) {
  ...
  addDailyHabit: async (name) => {
    ...
    if (get().supabaseReady) {
      const { error } = await insertDailyHabit(remoteMonthId, habit, dailyHabits.length - 1);
      if (error) {
  ...
  setMoodForDay: async (dayIndex, mood) => {
    ...
    if (get().supabaseReady) {
      const { error } = await upsertMood(remoteMonthId, dayIndex + 1, mood);
      if (error) {
  ...
  updateJournalEntry: async (dayIndex, entry) => {
    ...
    if (get().supabaseReady) {
      const { error } = await upsertJournal(remoteMonthId, dayIndex + 1, entry);
      if (error) {
  ...
  addWeeklyGoal: async (week, text) => {
    ...
    if (get().supabaseReady) {
      const { error } = await insertWeeklyGoal(
        remoteMonthId,
        weeklyGoal,
